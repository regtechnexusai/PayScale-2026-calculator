...
function calculate(grade, current) {
  ...
  const fullBasic = fixed
    ? applied
    : (scale.new[scale.new.indexOf(annualIncrementBasic) + 1] || annualIncrementBasic);
  ...
  return {
    ...
    fullBasic,
    phase1: fixed ? applied : current + Math.round(transitionIncrease * rates.phase1 / 100),
    phase2: fixed ? applied : current + Math.round(transitionIncrease * rates.phase2 / 100),
    phase3: fullBasic,
  };
}
...
for (const [gradeText, scale] of Object.entries(SCALES)) {
  ...
  for (const current of scale.old) {
    const result = calculate(grade, current);
    assert.ok(result.applied >= result.candidate || result.candidate === null, `Grade ${grade} listed-step fixation`);
    assert.ok(result.annualIncrementBasic >= result.applied, `Grade ${grade} 1 July 2026 increment`);
    assert.ok(result.fullBasic >= result.annualIncrementBasic, `Grade ${grade} 1 July 2027 increment`);
    assert.ok(result.phase1 <= result.phase2 && result.phase2 <= result.phase3, `Grade ${grade} phase order at ${current}`);
  }
...
// Gazette Article 9(2) audit case: fixation lands at 49,700, the one
// increment on 1 July 2026 moves the transition base to 52,200, and the
// 1 July 2027 full-pay stage includes the next annual increment at 54,800.
assert.deepEqual(calculate(10, 31780), {
  candidate: 47780,
  applied: 49700,
  annualIncrementBasic: 52200,
  fullBasic: 54800,
  phase1: 41990,
  phase2: 47095,
  phase3: 54800,
